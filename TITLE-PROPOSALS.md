...
| - | - | - | - | - |
| 1 | UC | @UofCincy Social  | LOW | Bare slug, no title info. "UC" = University of Cincinnati (per event name + `@uofcincy` handle), but the actual video subject is unknown. Proposed a light expansion; **needs Miles input: what is this reel?** Safe fallback = keep "UC". |
...
| 2 | Students Black Friday Discount | Creative Cloud for Students Black Friday Discount | HIGH | Clean, specific, offer-oriented. Keep as-is. |
...
| 2 | Premiere Pro Real Life Features | IBC Premiere Pro Release (Customer Interviews) | MED | Trims to a punchier phrasing; "Real Life" → "IRL" is optional. Safe fallback = keep original (also HIGH-quality). |
...
| \# | Current | Proposed | Confidence | Why |
| - | - | - | - | - |
| 1 | Watercolor Master Sneaks Interview | Project Watercolor Master: Adobe Researcher Sneaks Interview | HIGH | Clean; added colon for readability. Keep essence. |
| 2 | Project Type Lab Sneaks Interview | Project Type Lab: Adobe Researcher Sneaks Interview | HIGH | Same light punctuation cleanup. |
| 3 | Animations & Presets Sneaks Interview | Animations & Presets: Adobe Researcher Sneaks Interview | HIGH | Same. |
| 4 | In Office Event Trivia | Adobe MAX 2025: In-Office MAX Trivia | HIGH | Hyphenate "In-Office"; otherwise clean. |
...
| 6 | Interactive Event Activation Coverage | (TBD, was this MAX Acrobat? )Interactive Event Activation | HIGH | "Coverage" is filler; trimmed. |
| 7 | Reaction to SNEAKS in One Emoji | Event Recap Reactions: Sneaks in One Emoji | HIGH | Tighter; drops "Reaction to". |
| 8 | Defining a Creative Video Interview Collage | (need more here TBD) Defining Creativity: Interview Collage | MED | Word-salad slug. Best read: a collage of interviews answering "what does creative mean." Rephrased to that. **Confirm subject** — else keep LOW. |
| 9 | Photoshop Interview Demo | Photoshop Feature  Demo | HIGH | Clean, product-specific. Keep. |
| 10 | Premiere Pro Interview Demo | Premiere Pro  Demo | HIGH | Clean. Keep. |
| 11 | Gatorade Activation & Partnership Video | Adobe x Gatorade  Collaboration @AdobeMAX | HIGH | Drops "& Partnership Video" filler; keeps the brand + activation. |
...
| \# | Current | Proposed | Confidence | Why |
| - | - | - | - | - |
| 1 | NAB Emoji Reaction Interviews | NAB ‘24: Emoji Reaction Interviews | HIGH | Clean; light punctuation. |
| 2 | NAB Premiere Pro AI Feature Interviews | NAB ‘24: Premiere Pro AI Announcement Reactions | HIGH | Product-forward reorder; trims "Feature". |
| 3 | NAB Audio Enhancements Real-Time Testing | NAB ‘24: Premiere Enhanced Speech Live Test | MED | Tighter; "Real-Time Testing" → "Live Test". Fallback = keep original. |
| 4 | NAB Day in the Life Event Recap | NAB ‘24: Day-in-the-Life Recap | HIGH | Hyphenate; drop redundant "Event". |
...
| \# | Current | Proposed | Confidence | Why |
| - | - | - | - | - |
| 1 | Upworthy | TacoBell x Upworthy Feature | LOW | Bare slug = the publisher/handle (`@upworthy`), not a title. No subject info in the data. **Needs Miles input: what is this reel about?** Safe fallback = keep "Upworthy". |
...
| \# | Current | Proposed | Confidence | Why |
| - | - | - | - | - |
| 1 | Coca Cola Activation Interview | Adobe Summit ‘25: Coca-Cola Activation  | HIGH | Hyphenate brand name; otherwise clean. |
| 2 | Over & Under AI Enterprise Activity | Over/Under: (TBD was this hot takes? If so Summit ‘25 Hot Takes: B2B ) Enterprise AI Game  
  
If Trviai Game :   
“Over & Under: AGI Trivia” | MED | "Activity" → "Game"; tightened. Keeps the "Over & Under" mechanic. Fallback = keep original. |
| 3 | Adobe Acrobat Escape Room Activity | Summit  ‘25: Acrobat Escape Room  | HIGH | Drops redundant "Adobe" + "Activity". |
...
| 5 | Talent Marketing Best Job | Summit  ‘25: Coolest Job @Adobe S1 | MED | Ambiguous slug; likely the "coolest/best job" segment for Adobe Life recruiting (`@adobelife`). Fallback = keep original. |
| 6 | Adobe Summit Reactions Recap | Summit ‘25: Sneaks Emoji Reactions   | HIGH | Drops redundant "Adobe". |
| 7 | Adobe Summit Event Recap | Summit ‘25: Hosted Event Recap | HIGH | Drops "Adobe"; adds year for specificity. |
| 8 | Celebrity Interview Game | Summit ‘25: Ken Jeong Interview  | HIGH | Clean and specific. Keep. |
| 9 | Summit Hot Takes | Summit ‘25: Escalator ‘Hot’ Takes | HIGH | Clean, punchy. Keep. |
...
| \# | Current | Proposed | Confidence | Why |
| - | - | - | - | - |
| 1 | Dave Werner | Dave Employee Comms Interview | MED | NOT MAX Bare name, but event = celebrity/creator interview series and reel already carries a `role` field. "Interview" is well-supported by the playlist pattern. Fallback = keep "Dave Werner". |
| 2 | Produced and Storyboarded Bowen | Bowen Employee Comms Interview | HIGH | **NOT MAX Production note leaked into title.** "Produced and Storyboarded" belongs in `role` (already set to "In-house production — produced & creatively directed"). Clean title = subject name + segment. |
| 3 | Mansa | Mansa Employee Comms Interview | MED | NOT MAX Bare name; same interview-series pattern (has `role` field). Fallback = keep "Mansa". **Confirm this is an interview** (who is Mansa?). |
|  |  |  |  |  |
...
- **Kelley O'hara → Kelley O'Hara** — capitalization correction in the source data (`title` and the `Kelley-Ohara\_...mp4` path both use "Ohara"; the path can stay, but the display title should read "O'Hara").
